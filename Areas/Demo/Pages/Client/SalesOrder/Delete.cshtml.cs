...
public class DeleteModel(AppDbContext context, ILogger<DeleteModel> logger)
    : PageModel {
#region ctor
...
#region properties
  ...
  /// <summary>
  /// Gets or sets the collection of sales orders associated with the
  /// application.
  /// </summary>
...
#region methods
  ...
  /// <returns>An IActionResult that renders the page if successful, or NotFound
  /// if the ID is invalid.</returns>
  public async Task<IActionResult> OnGetAsync(int? id) {
    if (id is null) {
      var errorMessage =
          $"{_className}: Id {id} or SalesOrder context {_context.ContextId} is null";
  ...
  /// <returns>An IActionResult that redirects to the Details page if
  /// successful, or NotFound if the ID is invalid.</returns>
  [Obsolete]
  public async Task<IActionResult> OnPostAsync(int? id) {
    ...
    try {
      ...
      if (client.RequestInterceptor is null) {
        ...
        var so =
            await client
                .GetByKeysAsync<Acumatica.Default_25_200_001.Model.SalesOrder>(
                    keys, select: "OrderType, OrderNbr, Status");
        if (so is null) {
          var errorMessage =
              $@"{_className}: Sales Order {keys} not found in Acumatica ERP.";
          _logger.LogError(errorMessage);
          ...
          if (StatusList.Contains(so.Status)) {
            ...
            var infoMessage =
                $"{_className}: Sales Order {keys} is in a valid status for deletion. Status is {so.Status}.";
            _logger.LogInformation(infoMessage);
          } else {
            // if the status is not in the list, we cannot delete the Sales
            // Order
            var errorMessage =
                $"{_className}: Sales Order {so.OrderType} {so.OrderNbr} is not in a valid status for deletion. Status is {so.Status}.";
            _logger.LogError(errorMessage);
        ...
        var result =
            client.DeleteAsync<Acumatica.Default_25_200_001.Model.SalesOrder>(
                so);
        if (result is not null) {
          var infoMessage =
              $"{_className}: Sales Order {so.OrderNbr} deletion status is {result}.";
          _logger.LogInformation(infoMessage);
        }

        // remove salesOrder from SalesOrder_App cache
        _ = _context.Remove(SalesOrder);
        _ = _context.SaveChanges();

        var infoMessage2 =
            $"{_className}: SalesOrder deleted from local store. {SalesOrder}";
        _logger.LogInformation(infoMessage2);

        // set the DeleteFlag to true to indicate that the Sales Order has been
        // deleted from local store and Acumatica ERP
        Globals.SetGlobalProperty("DeleteFlag", true, _logger);
      }
    } catch (Exception ex) {
      _logger.LogError(
          $"{_className}: Failed to delete Sales Order exception {ex}.");
      return RedirectToPage("./Details");
    } finally {
